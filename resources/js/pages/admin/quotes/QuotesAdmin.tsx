...
import { Mail, Check, Search, RefreshCw, ChevronDown, ChevronUp, CheckCheck } from 'lucide-react';
...
export default function AdminQuotesPage() {
    ...
    const answeredQuotes = filteredQuotes.filter(q => q.is_answered).length;
    const pendingQuotes = filteredQuotes.filter(q => !q.is_answered).length;
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="min-h-screen bg-gray-50 py-8 md:px-6 p-4">
                ...
                <div className="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <StatCard
                        title="Answered"
                        value={answeredQuotes}
                        icon={<Check className="h-5 w-5 text-white" />}
                        color="from-green-400/80 to-green-600/80"
                        glow="shadow-green-500/30"
                        trend="up"
                        percentage={totalQuotes > 0 ? `${Math.round((answeredQuotes / totalQuotes) * 100)}%` : "0%"}
                    />
                    <StatCard
                        title="Pending"
                        value={pendingQuotes}
                        ...
                        trend="down"
                        percentage={totalQuotes > 0 ? `${Math.round((pendingQuotes / totalQuotes) * 100)}%` : "0%"}
                    />
                </div>
                    ...
                    <div className="p-4 border-b border-gray-200">
                        <div className="flex flex-col md:flex-row md:items-center justify-between space-y-4 md:space-y-0">
                            ...
                            <div className="text-sm text-gray-500">
                                Showing {currentQuotes.length} quotes
                            </div>
                        </div>
                    </div>
                                        ...
                                        {currentQuotes.map((quote) => (
                                            <tr key={quote.id} className={quote.is_answered ? 'bg-gray-50' : 'bg-white'}>
                                                ...
                                                <td className="px-3 py-4 whitespace-nowrap text-right text-sm flex gap-5 items-center font-medium">
                                                    {!quote.is_answered ? (
                                                        <CheckCheck onClick={() => window.location.href = route('admin.quotes.EmailQuotes', quote.id)} className="h-5 w-5 text-gray-600 hover:text-gray-900" />
                                                    ) : (
                                                        <CheckCheck onClick={() => markAsAnswered(quote.id)} className="h-5 w-5 text-green-600 hover:text-green-900" />
                                                    )}
                                                </td>
                                            </tr>
                                        ))}
